...
import { Nomnoml, DocsifyPlugin, DocsifyHook, DocsifyInstance } from './types'
...
describe('when invalid input is passed to the render interceptor', () => {
	...
	beforeEach(() => {
		...
		logFn = jest.spyOn(global.console, 'error').mockImplementation(() => {
			/* silence is golden */
		})
	})
	...
	`(
		...
		({ language }) => {
			...
			expect(defaultInterceptor).toBeCalledWith('string', language, false)
		}
	)
...
describe('if the nomnoml dependency returns an something unexpected', () => {
	let logFn: any
	beforeEach(() => {
		;(nomnoml as Nomnoml).renderSvg = () => '<unknown/>'
		logFn = jest.spyOn(global.console, 'error').mockImplementation(() => {
			/* silence is golden */
		})
	})

	it('should log an error message for returned exceptions', () => {
		const { mockVm } = getMockVm()
		plugin(mockHook, mockVm)
		callInterceptor(mockVm, '[A] -> [B]', 'noml render', false)
		expect(logFn).toHaveBeenCalledWith(
			expect.stringContaining('[docsify-nomnoml]'),
			expect.any(Object)
		)
	})
})
